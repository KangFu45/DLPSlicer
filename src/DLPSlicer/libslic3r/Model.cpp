...
Model::_arrange(const Pointfs &sizes, coordf_t dist, const BoundingBoxf* bb, Pointfs &out) const
{
    ...
    bool result = Slic3r::Geometry::arrange(
        ...
        bb,                         // bounding box of the area to fill		在这个包围盒内填充
        out                         // output positions	输出的位置
    );
...
/*  arrange objects preserving保存 their instance count
    but altering变更 their instance positions */
Pointfs
Model::arrange_objects(coordf_t dist, const BoundingBoxf* bb)
{
   ...
   // for (ModelObjectPtrs::const_iterator o = this->objects.begin(); o != this->objects.end(); ++o) {
   //     for (ModelInstancePtrs::const_iterator i = (*o)->instances.begin(); i != (*o)->instances.end(); ++i) {
   //         (*i)->offset = positions.back();//将所有实例在xy方向上重新排列
   //         positions.pop_back();
   //     }
   //     (*o)->invalidate_bounding_box();
   // }
...
void Model::arrange(const Pointfs& sizes, coordf_t dist, const BoundingBoxf* bb, Pointfs& out)
{
	...
	std::map<int, Pointf> posID;//存储位置和id

	std::vector<Linef> beginLine;//长边（由底到高排序）
	beginLine.push_back(Linef(bb->min, Pointf(10000, bb->min.y)));//10000为延伸的最远距离

	//std::vector<Linef> leftConfine;//左侧边界
	...
	for (int i = 0; i < sizes.size(); ++i) {
		...
		if (temp > 0) {
			...
			for (auto b = beginLine.begin(); b != beginLine.end(); ++b) {
				...
				auto s1 = sizes1.begin() + dis;//获取x最长边模型方框

				if (b == beginLine.end() - 1) {//判断是否为最后一条起始边
					//放入x最长边模型
