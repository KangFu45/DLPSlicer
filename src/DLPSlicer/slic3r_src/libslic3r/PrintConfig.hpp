...
enum InfillPattern {
    ...
    ipHilbertCurve, ipArchimedeanChords, ipOctagramSpiral,ip3DSupport,
};
...
template<> inline t_config_enum_values ConfigOptionEnum<InfillPattern>::get_enum_values() {
	...
	keys_map["3Dsupport"]			= ip3DSupport;
    return keys_map;
}
    ...
    : public virtual StaticPrintConfig
{
    ...
    ConfigOptionFloat               layer_height;
    ConfigOptionInt                 raft_layers;
    ConfigOptionInt                 raft_offset;

	ConfigOptionFloat				support_top_height;
    ConfigOptionFloat				support_radius;
	ConfigOptionFloat				support_top_radius;
	ConfigOptionFloat				support_bottom_radius;
	ConfigOptionInt					space;
	ConfigOptionInt					angle;
	ConfigOptionInt					leaf_num;
	ConfigOptionFloat   			model_lift;

	ConfigOptionInt                 overLayer;
	ConfigOptionInt                 overIlluTime;
	ConfigOptionInt					over_inttersity;
	ConfigOptionInt					norm_inttersity;
	ConfigOptionInt                 normIlluTime;
	ConfigOptionInt                 firstIllu;

	ConfigOptionFloat				wall_thickness;
	ConfigOptionBool				hollow_out;
	ConfigOptionFloat				arrange_space;
    ...
    virtual ConfigOption* optptr(const t_config_option_key &opt_key, bool create = false) {
        ...
        OPT_PTR(raft_layers);
        OPT_PTR(raft_offset);
        OPT_PTR(support_radius);
		OPT_PTR(support_top_radius);
		OPT_PTR(support_bottom_radius);
		OPT_PTR(overLayer);
		OPT_PTR(overIlluTime);
		OPT_PTR(over_inttersity);
		OPT_PTR(norm_inttersity);
		OPT_PTR(normIlluTime);
		OPT_PTR(firstIllu);
		OPT_PTR(space);
		OPT_PTR(angle);
		OPT_PTR(wall_thickness);
		OPT_PTR(hollow_out);
		OPT_PTR(arrange_space);
		OPT_PTR(leaf_num);
        OPT_PTR(threads);
		OPT_PTR(model_lift);
		OPT_PTR(support_top_height);
