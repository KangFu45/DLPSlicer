...
Vectorf3 Pointf3::Cross(const Vectorf3& v) const
{
	return Vectorf3(
		y * v.z - z * v.y,
		z * v.x - x * v.z,
...
void Pointf3::rotate_x(double angle)
{
	...
	double xold = this->y;
	double yold = this->z;
	this->y = c * xold - s * yold;
	this->z = s * xold + c * yold;
}

void Pointf3::rotate_y(double angle)
{
	double radian_angle = (angle / 180.0) * PI;
	double c = cos(radian_angle);
	double s = sin(radian_angle);
	...
	double xold = this->x;
	double yold = this->z;
	this->x = c * xold - s * yold;
	this->z = s * xold + c * yold;
}

void Pointf3::rotate_y_radian(double angle)
{
	double c = cos(angle);
	double s = sin(angle);
...
void Pointf3::rotate_z_radian(double angle)
{
	//double radian_angle = (angle / 180.0) * PI;
	double c = cos(angle);
	double s = sin(angle);
