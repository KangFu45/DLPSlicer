...
stl_verify_neighbors(stl_file *stl) {
  ...
  //为边的指针分配内存
//  stl->edge_start = (stl_edge*)
//	  calloc(stl->stats.number_of_facets * 3 / 2, sizeof(stl_edge));
//  if (stl->edge_start == NULL) perror("stl_generate_shared_vertices");
//
//  for (int z = 0; z < stl->stats.number_of_facets * 3 / 2; ++z) {
//   stl->edge_start[z].facet_number1 = -1;
//   stl->edge_start[z].facet_number2 = -1;
//  }
  ...
  for(i = 0; i < stl->stats.number_of_facets; i++) {
    for(j = 0; j < 3; j++) {
	 ...
	 //   //找到相邻面的共边
	 //    if (neighbor > i) {
	 //     stl->edge_start[edge].p1 = edge_a.p1;
	 //     stl->edge_start[edge].p2 = edge_a.p2;
	 //     stl->edge_start[edge].facet_number1 = i;
	 //     stl->edge_start[edge].facet_number2 = neighbor;
	 //     ++edge;
	 //    }
	 // }	 
    }
