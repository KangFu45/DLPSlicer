...
stl_check_facets_exact(stl_file *stl) {
  /* This function builds the neighbors list.这个函数基于相邻列表  No modifications are made
   *  to any of the facets.对所有面没有修改  The edges are said to match only if all six
   *  floats of the first edge matches all six floats of the second edge第一条边的六个数与第二条边的六个数匹配，这些边才能匹配.
   */
  ...
  for(i = 0; i < stl->stats.number_of_facets; i++) {
    ...
    // Positive正 and negative负 zeros are possible in the floats, which are considered equal by the FP unit.
    ...
    /*判断面上的点与点是否相同，相同则退化删除 If any two of the three vertices are found to be exactally the same, call them degenerate and remove the facet. */
...
stl_load_edge_exact(stl_file *stl, stl_hash_edge *edge,
  ...
  // Ensure identical vertex ordering of equal edges.确保相同的点依照共边排序
  // This method is numerically robust.这是一种粗鲁的数学方法
  if ((a->x != b->x) ? (a->x < b->x) : ((a->y != b->y) ? (a->y < b->y) : (a->z < b->z))) {
    ...
    edge->which_edge += 3; /* this edge is loaded backwards 反方向 */
  }
}
...
insert_hash_edge(stl_file *stl, stl_hash_edge edge,
  ...
  if(link == stl->tail) {
    /* 列表还没有边 This list doesn't have any edges currently in it.  Add this one. */
    ...
    *new_edge = edge;
    ...
    /* This is a match.  Record记录 result in neighbors list. */
    ...
    for(;;) {
      if(link->next == stl->tail) {
        /* 列表最后一个元素 This is the last item in the list. Insert a new edge. */
        new_edge = (stl_hash_edge*)malloc(sizeof(stl_hash_edge));
        if(new_edge == NULL) perror("insert_hash_edge");
        stl->stats.malloced++;
        *new_edge = edge;
...
stl_compare_function(stl_hash_edge *edge_a, stl_hash_edge *edge_b) {
  if(edge_a->facet_number == edge_b->facet_number) {
    return 1;			/* 相同的面不要匹配边 Don't match edges of the same facet */
  } else {
...
stl_check_facets_nearby(stl_file *stl, float tolerance) {
  ...
  if(   (stl->stats.connected_facets_1_edge == stl->stats.number_of_facets)
    ...
    /*不需要进一步检查，所有面都相连 No need to check any further.  All facets are connected */
    return;
  }
...
stl_record_neighbors(stl_file *stl,
  ...
  if(   ((edge_a->which_edge < 3) && (edge_b->which_edge < 3))
    ...
    /*这些面都是反方向 these facets are oriented in opposite directions.  */
    /* 它们的法向量很可能弄混了 their normals are probably messed up. */
    stl->neighbors_start[edge_a->facet_number].
...
stl_fill_holes(stl_file *stl) {
  ...
  /*将所有未连接的边插入哈希表 Insert all unconnected edges into hash list */
  stl_initialize_facet_check_nearby(stl);
  for(i = 0; i < stl->stats.number_of_facets; i++) {
    facet = stl->facet_start[i];
    for(j = 0; j < 3; j++) {
